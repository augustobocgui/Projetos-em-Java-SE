...
public class verificar_CPF {
    ...
    public void verificando() {
        ...
        int digitoCPF1, digitoCPF2, erro;
        char CPF[] = getCpf().toCharArray();
        digitoCPF1 = digitoCPF2 = erro = 0;
        ...
        soma = ((s1 * 11) + (s2 * 10) + (s3 * 9) + (s4 * 8) + (s5 * 7) + (s6 * 6) + (s7 * 5) + (s8 * 4) + (s9 * 3) + (digitoCPF1 * 2));
        resto = (soma % 11);
        if (resto < 2) {
            digitoCPF2 = 0;
        } else {
